...
cd-all:
    ...
    (cd zig && zig build -Dtarget=x86_64-linux-musl -Doptimize=ReleaseFast && cp zig-out/bin/unsafie-cloud ../dist/unsafie-cloud-x86_64-linux) &
    PID1=$!
    (cd zig && zig build -Dtarget=aarch64-linux-musl -Doptimize=ReleaseFast && cp zig-out/bin/unsafie-cloud ../dist/unsafie-cloud-aarch64-linux) &
    PID2=$!
    (cd zig && zig build -Dtarget=x86_64-windows -Doptimize=ReleaseFast && cp zig-out/bin/unsafie-cloud.exe ../dist/unsafie-cloud-x86_64.exe) &
    PID3=$!
    (cd zig && zig build -Dtarget=aarch64-macos -Doptimize=ReleaseFast && cp zig-out/bin/unsafie-cloud ../dist/unsafie-cloud-aarch64-macos) &
    PID4=$!
    (cd zig && zig build -Dtarget=x86_64-macos -Doptimize=ReleaseFast && cp zig-out/bin/unsafie-cloud ../dist/unsafie-cloud-x86_64-macos) &
    PID5=$!
    ...
    PID7=$!
    (python3 -m pip install build --quiet 2>/dev/null || true; python3 -m build python/ -o dist/ 2>/dev/null || true) &
    PID8=$!
