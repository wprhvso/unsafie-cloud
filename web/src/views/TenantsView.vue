...
<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-bold text-white">K3s Tenants & vcluster</h1>
        <p class="text-slate-400 text-sm mt-1">Isolated namespaces and virtual clusters</p>
      </div>
    ...
    <div v-if="tenants.length === 0" class="bg-slate-900 border border-slate-800 p-12 text-center rounded-2xl text-slate-400">
      No tenants created. Click above to deploy one.
    </div>
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div v-for="t in tenants" :key="t.id" class="bg-slate-900 border border-slate-800 p-6 rounded-2xl space-y-4">
        ...
        <div class="text-sm text-slate-400 space-y-1">
          <div>Quotas: {{ t.cpu_limit_cores }} vCPU / {{ Math.round(t.ram_limit_mb / 1024) }} GB RAM / {{ t.storage_limit_gb }} GB Storage</div>
        ...
        <div class="pt-2 border-t border-slate-800 flex justify-between items-center text-xs">
          <span class="text-slate-500">Status: <strong class="text-emerald-400">{{ t.status }}</strong></span>
    ...
    <div v-if="showCreateModal" class="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50">
      <div class="bg-slate-900 border border-slate-800 rounded-2xl max-w-md w-full p-6 space-y-4">
        <h3 class="text-lg font-bold text-white">Create K3s Tenant</h3>
        <div class="space-y-3 text-sm">
          <div>
            <label class="block text-slate-400 mb-1">Project / Team Name</label>
          ...
          <div>
            <label class="block text-slate-400 mb-1">Isolation Type</label>
        ...
        <div class="flex justify-end space-x-3 pt-4 border-t border-slate-800">
          <button @click="showCreateModal = false" class="px-4 py-2 bg-slate-800 text-slate-300 rounded-xl text-sm font-medium">Cancel</button>
